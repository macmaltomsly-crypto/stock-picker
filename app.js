...
function sigChips(s){
  return Object.keys(SIG).filter(function(k){return s[k]&&s[k].length;}).map(function(k){
    return '<span class="sc-chip '+SIG[k].c+'" title="'+s[k].slice(0,3).join(', ')+'">'+SIG[k].l+'</span>';
  }).join('');
}
...
function bCls(){return 'buy-btn'+(MODE==='paper'?' paper':'');}
function gCls(){return 'gem-buy'+(MODE==='paper'?' paper':'');}
function sCls(){return 'ss-buy'+(MODE==='paper'?' paper':'');}
function bLbl(sym){return 'Buy $'+ORDER_AMT+(MODE==='paper'?' (P)':'')+(sym?' of '+sym:'');}
